...
/obj/item/circuitboard/machine/examine(mob/user)
	...
	if(LAZYLEN(req_components))
		...
		var/list/already_have = user.GetAllContents()  // get only the items you have
		nice_list = req_components

		for(var/atom/part in already_have)
			if(part in already_have)
				var/new_amt = req_components[part] - already_have[part]
				if(new_amt >= 0 && !ispath(part))
					nice_list += list("[req_components[part]] [initial(part.name)]")
